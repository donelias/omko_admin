...
class ApiController extends Controller
{
    ...
    public function post_property(Request $request)
    {
        ...
        try {
            ...
            $result = Property::with('customer')->with('category')->with('assignfacilities.outdoorfacilities')->with('favourite')->with('parameters')->with('interested_users')->where('id', $saveProperty->id)->get();
        ...
        try {
            ...
            if (($property)) {
                ...
                if ($action_type == 0) {
                    ...
                    $property->save();
                    $update_property = Property::with('customer')->with('category')->with('assignfacilities.outdoorfacilities')->with('favourite')->with('parameters')->with('interested_users')->where('id', $request->id)->get();
    ...
    public function get_articles(Request $request)
    {
        $offset = isset($request->offset) ? $request->offset : 0;
        $limit = isset($request->limit) ? $request->limit : 10;
        $article = Article::with('category','category.translations' ,'translations')->select('id', 'slug_id', 'image', 'title', 'description', 'view_count', 'meta_title', 'meta_description', 'meta_keywords', 'category_id', 'created_at');
    ...
    public function get_favourite_property(Request $request)
    {
        ...
        $property_details = Property::whereIn('id', $arr)->where(['request_status' => 'approved', 'status' => 1])->with('category','category.translations', 'translations')->with('assignfacilities.outdoorfacilities')->with('parameters');
    ...
    public function user_interested_property(Request $request)
    {
        ...
        $property_details = Property::whereIn('id', $arr)->with('category')->with('parameters');
        ...
        if (!$validator->fails()) {
            ...
            if ($language) {
                if ($request->web_language_file) {
                    $json_file_path = public_path('web_languages/' . $languageCode . '.json');
                } else {
                    $json_file_path = public_path('languages/' . $languageCode . '.json');
                }
    ...
    public function get_user_recommendation(Request $request)
    {
        ...
        if (collect($user_interest)->isNotEmpty()) {
            ...
            $property = Property::with('customer')->with('user')->with('category','category.translations')->with('assignfacilities.outdoorfacilities')->with('favourite')->with('parameters')->with('interested_users')->where(['status' => 1, 'request_status' => 'approved']);
    ...
    public function post_project(Request $request)
    {
        ...
        try {
            ...
            $result = Projects::with('customer')->with('gallary_images')->with('documents')->with('plans')->with('category')->where('id', $project->id)->get();
        ...
        try {
            ...
            if ($request->get_similar == 1) {
                ...
                $similarProjectQuery = Projects::select('id', 'slug_id', 'city', 'state', 'country', 'title', 'type', 'image', 'location','category_id','added_by','request_status')->where(function($query){
                    $query->where(['request_status' => 'approved','status' => 1]);
                })->with('category')->with('category.translations','translations');
                if($request->has('id') && !empty($request->id)){
    ...
    public function getAgentTimeSchedules(){
        try{
            ...
            $agentTimezone = $loggedInUser->timezone ?? 'UTC';
    ...
    public function updateLanguage(Request $request){
        try {
            ...
            $languageCodeValue = $request->language_code;
            if($languageCodeValue == 'en'){
                $languageCodeValue = 'en-new';
            }
            $language = Language::where(['code' => $languageCodeValue, 'status' => 1])->count();
            if(!$language){
                ApiResponseService::errorResponse(trans("Language not found"));
            }
            $languageCodeNew = $request->language_code;
            if($languageCodeNew == 'en'){
                $languageCodeNew = 'en-new';
            }
            $loggedInUser = Auth::user();
            Customer::where('id', $loggedInUser->id)->update(['default_language' => $languageCodeNew]);
