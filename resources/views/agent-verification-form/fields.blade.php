...
@section('title')
    {{ (isset($form_type) && $form_type == 'become_agent') ? __('User Verification Form Fields') : __('Agent Verification Form Fields') }}
@endsection
        ...
        <section class="section">
            ...
            <div class="card-content">
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            {!! Form::open(['url' => route('agent-verification-form-fields.store'), 'data-parsley-validate', 'files' => true, 'class' => 'create-form','data-pre-submit-function','data-success-function'=> "formSuccessFunction"]) !!}
                                @csrf

                                <div class="row">
                                    {{-- Section --}}
                                    <div class="col-sm-12 col-md-4 form-group mandatory">
                                        {{ Form::label('section', __('Section'), ['class' => 'form-label text-center']) }}
                                        <select name="agent_verification_form_section_id" id="section-id" class="form-select form-control-sm" data-parsley-required=true>
                                            <option value="">{{ __('Select Section') }}</option>
                                            @foreach($sections as $section)
                                                <option value="{{ $section->id }}">{{ $section->name }} ({{ __(ucwords(str_replace('_', ' ', $section->form_type))) }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    {{-- Name --}}
                                    <div class="col-sm-12 col-md-4 form-group mandatory">
                                        {{ Form::label('type', __('Field Name'), ['class' => 'form-label text-center']) }}
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div id="toolbar" class="d-flex align-items-center mb-3 flex-wrap gap-2">
                                <select id="filter_form_type" class="form-select w-auto me-2">
                                    <option value="">{{ __('All Form Types') }}</option>
                                    <option value="become_agent" {{ (isset($form_type) && $form_type == 'become_agent') ? 'selected' : '' }}>{{ __('Become an Agent') }}</option>
                                    <option value="verify_agent" {{ (isset($form_type) && $form_type == 'verify_agent') ? 'selected' : '' }}>{{ __('Verify Agent') }}</option>
                                </select>
                                <select id="filter_section" class="form-select w-auto me-2">
                            ...
                            <table class="table table-striped"
                                id="table_list" data-toggle="table" data-url="{{ route('agent-verification-form-fields.show') }}"
                                ...
                                <thead class="thead-dark">
                                    <tr>
                                        ...
                                        <th scope="col" data-field="agent_verification_form_section.name" data-sortable="true">{{ __('Section') }}</th>
                                        <th scope="col" data-field="name" data-sortable="true">{{ __('Field Name') }}</th>
    ...
    <div id="editModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="verifyCustomerFormEditModal"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form class="form-horizontal create-form" action="{{ route('agent-verification-form-fields.update') }}" enctype="multipart/form-data" data-parsley-validate data-success-function="editFormSuccessFunction">
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6 form-group mandatory">
                                {{ Form::label('section', __('Section'), ['class' => 'form-label text-center']) }}
                                <select name="agent_verification_form_section_id" id="edit-section-id" class="form-select form-control-sm" data-parsley-required=true>
                                    <option value="">{{ __('Select Section') }}</option>
                                    @foreach($sections as $section)
                                        <option value="{{ $section->id }}">{{ $section->name }} ({{ __(ucwords(str_replace('_', ' ', $section->form_type))) }})</option>
                                    @endforeach
    ...
    <script>
        $(document).ready(function() {
            $(document).on('change keyup','.option-input',function() {
                checkDuplicateFeatures('.option-input','.add-new-option');
            });

            $(document).on('change keyup','.edit-option-input',function() {
                checkDuplicateFeatures('.edit-option-input','.add-new-edit-option');
            });
        ...
        function queryParams(p) {
            return {
                ...
                limit: p.limit,
                search: p.search,
                filter_section_id: $('#filter_section').val(),
        ...
        window.actionEvents = {
            'click .edit_btn': function(e, value, row, index) {
                ...
                $("#edit-section-id").val(row.agent_verification_form_section_id);
